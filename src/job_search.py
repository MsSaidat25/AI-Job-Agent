...
def _to_listing(job: dict, profile: UserProfile) -> JobListing:
    ...
    return JobListing(
        ...
        job_type        = (job.get("job_employment_type") or "FULL_TIME").upper(),
...
def _format_md(job: JobListing, index: int) -> str:
    icon   = "🟡" if job.match_score >= 80 else "🟢" if job.match_score >= 60 else "🔵"
    salary = (
    ...
    return (
        ...
        f"**Match:** {icon} {job.match_score}/100 — {job.match_rationale}\n"
        f"{desc}{apply}\n---"
    )
